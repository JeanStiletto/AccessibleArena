...
- **IRoundTripWorkflow** (`Wotc.Mtga.DuelScene.Interactions`): `CanHandleRequest`, `OnRoundTrip(req)`, `IsWaitingForRoundTrip()`, `CanCleanupAfterOutboundMessage`. Implemented by `SelectTargetsWorkflow` and friends. **Every accepted click is a server round trip**: `OnClick` → `ApplyTheSelection` → `UpdateTarget` sets `_submitted = true`; `IsWaitingForRoundTrip()` returns `_submitted`; `CanClick` returns false while it is set; `OnRoundTrip` clears it. Consequence for the mod: multi-click sequences must gate on `IsWaitingForRoundTrip()` + `CanClick`, never on a fixed timer — intermediate clicks are silently refused (invalid sfx only).
- **WorkflowBase.TryRerouteClick(instanceId, battlefieldHolder, isSelecting, out view)** (protected static): a click on any card of a battlefield stack is rerouted to `IBattlefieldStack.OldestCard` when selecting and `YoungestCard` when unselecting. Skipped by `CanRerouteClick` when the stack has attachments/exile, is a block stack, or the parent is distinguished. So clicking specific stack members only works if you walk them oldest-first (lowest InstanceId) and let the stack re-split between clicks — the selected copy leaves the stack because `IStackableWorkflow.CanStack` refuses to group cards whose `Target.LegalAction`/`Highlight` differ.
- **IClickableWorkflow**: `CanClick(IEntityView, SimpleInteractionType)`, `OnClick`, `CanClickStack(CdcStackCounterView, SimpleInteractionType)`, `OnClickStack`, `OnBattlefieldClick`. Parent workflows expose children via a `ChildWorkflows` property; `ActionsAvailableWorkflow` forwards to its `_currentVariant` itself.
- **PayCostWorkflow** (`IParentWorkflow`): runs costs paid *while casting* — ward, additional costs. Its `SetButtons()` merges child buttons and, when the children contribute none, promotes **Cancel to `ButtonStyle.StyleType.Main`** — so the scene's `PromptButton_Primary` reads "Cancel" and there is **no counted confirm button**. The child doing the actual picking is a `SelectionWorkflow` (SelectNRequest), reachable through `ChildWorkflows`; its `CanClick` → `IsSelectable(instanceId)` is the only reliable way to find the selectable cards, because the affected hand cards get no `HotHighlight` object in this state. Detecting "the game wants a card picked" by button count or by visual highlight both fail here — see `HotHighlightNavigator.GetPaymentClickTargets`.
- **SelectionWorkflow** (`Wotc.Mtga.DuelScene.Interactions.SelectN`): `SelectNHighlightsGenerator` gives selectable ids `HighlightType.Hot` normally, but `Cold` whenever `HotIds` is non-empty (or the Birthing Ritual special case) — so "selectable" does not always imply a HotHighlight. `OnClick` submits directly when min == max == 1, which is why a ward discard needs no confirm press.
