...
- Tab/Shift+Tab group cycling in the deck builder no longer breaks on quick key taps at low frame rates. The Unity 6 input port (v1.6) changed the "is Tab held" check semantics: the new Input System reports a key that was pressed and released within a single frame as *not* pressed (legacy input guaranteed at least one frame of "held"). On slower machines a brisk Tab tap therefore fell through to per-item navigation — Tab walked the deck list one card at a time and dead-ended at "End of list" instead of cycling Filters / Collection / Deck List / Deck Info. The Tab checks in the menu navigator and the EventSystem move-blocking patch now also accept a same-frame press. A second log from [sandraloga21-del](https://github.com/sandraloga21-del) (issue #122) showed the same fall-through on another machine: after a card search, Tab walked the filter row one control at a time and never reached the results, which read as "the deck builder cannot add cards".

- An audit for the same frame-rate trap fixed three more spots before anyone hit them: a quick Space tap in a duel could slip past the phase-skip warning and the browser Space-block (the game's Submit would fire even though the mod meant to intercept it), the friends-panel Tab-block could miss a deferred toggle, and a same-frame release-and-repress of an arrow key could swallow the new press in the hold-repeat tracker.
...
- The event page's win-reward ladder is finally spoken. Sighted players see a row of prize bubbles ("0 wins: 25 gold, 1 win: 50 gold, ... 7 wins: 2 packs + 1,000 gold") with hover popups; a screen reader got nothing, because the bubbles are filed under the home page's Objectives subgroup, which the event page never shows, and the event-info text sweep deliberately skips everything under an "Objective" parent. The mod now reads the ladder's reward data straight from the game's track component and offers it as a new "Rewards" group with one entry per tier: "3 wins: 1,000 Gold, 2 Booster Packs". The win count uses the game's own localized strings; the reward wording is the same popup text sighted players hover to read. Cumulative tracks mark reached tiers as "earned", by-course tracks mark the tier matching the current record as "current" — mirroring which bubble the game highlights. Asked for by the reporter of issue #122.

- The event page is organized like the deck builder now. The description text used to be injected as one top-level entry per paragraph; together with an eight-tier ladder a plain Bo3 event page would have become a 20-entry list. Description paragraphs now live in one "Event Info" group and the ladder in the "Rewards" group — Enter opens a group, Up/Down walk its entries, Backspace returns to the buttons — and Tab/Shift+Tab cycle entry button → Event Info → Rewards from anywhere on the page, so the Play or Pay button is never more than one Tab away. Entries inside the Info group no longer repeat the "Info:" prefix on every paragraph; the group name carries that context. The screen announcement gains a key hint for this, repeatable with Ctrl+F1.

- A pay button the game has greyed out now says so: "6000 gems, not available". The game disables the entry button until the event actually opens (and once a token entry is used up), while still printing the price on it — sighted players see the grey, a screen reader heard a normal price. Pressing Enter on it announced "Activated" and did nothing, because the activator's disabled-button guard only recognized the plain CustomButton type and the event page's buttons are the tooltip subclass. The guard now covers both, so Enter on any disabled tooltip button says "Item is disabled" instead of claiming success.
...
- The log now opens with a `Host:` line naming the MelonLoader version, the Unity version and the MTGA version. All three were previously only readable from MelonLoader's own log header, which is not always right — a Wine/Proton session reported Unity 2022.3.62f2 for a client that was demonstrably Unity 6. The Unity version is now read from the running engine, and the loader version matters because nightly loader builds (`0.7.2-ci.2388` and the like) are not a configuration we can reproduce.

- The installer now reads the whole release-tag segment when resolving the latest MelonLoader version, instead of stopping at the first non-digit. A tag such as `v0.7.4-rc1` was previously truncated to `v0.7.4`, and the download 404'd.
...
- Confirmation guards actually guard again (PR #121 by Michael Taboada, @lilmike — thanks!). Declaring no blockers spoke its "press again to confirm" prompt but carried on and declared no blockers immediately, and the confirmation for spending a draft token was reset every frame, so it could never be passed — an event could not be entered with a token at all. A follow-up closed the one remaining hole of the same kind: the Space shortcut that clicks the duel's primary prompt button had lost its phase-skip warning check in the v1.6 input-port rewrite (the old protection lived in a patch that the port removed), so Space could act through a warning that was never spoken. That path now asks the guard first, like every other confirm path.

- An empty deck no longer hides the Deck List group in the deck builder (#120). Groups without entries are normally dropped, so a freshly created deck offered filters and collection but no deck list at all — which reads as a broken screen when you cannot see that the list area is simply empty. The group now stays in the Tab cycle with a single entry saying the deck has no cards yet and how to add one, in all 12 locales; it disappears again the moment the first card is added. The log now also states why a deck list came back empty (genuinely empty deck, sideboard blade active, or the game-side holder missing — the last one is a loud warning, so a future game rename can never masquerade as an empty deck). Thanks to @Daggershade for the report and the log.
...
- The Sideboard header now says how many cards are in it — "Sideboard, 4 cards". An empty sideboard is the state you are in the moment you switch the toggle on, and with no cards there was no group and therefore no feedback at all; the count means the screen answers you either way.

- The card count is announced after sideboard edits too. Adding or removing a card normally reads back the new deck total, but nothing was ever spoken while the Sideboard toggle was on. The count is read off the main deck's header, and the game only hides that header's button when it switches to the sideboard — the header itself keeps displaying the unchanged main deck total, so every sideboard edit looked like "nothing changed" and the announcement was suppressed. While the toggle is on you now hear the sideboard's own count instead. The Sideboard header line counts the same way, and both count copies rather than rows, so a card sitting at three copies counts as three.

- Ctrl+Enter on a card in the deck list or the sideboard adds one more copy, without going back to the collection. Every row in the deck blade is really two buttons — the card name removes a copy, the "4x" quantity chip next to it adds one — and only the first of the two was ever reachable, so bumping a card from two copies to three meant leaving the deck list, finding the card in the collection and coming back. Ctrl+Enter now presses the chip, which is the same control a sighted player clicks. Enter still removes a copy. The copy lands in whichever pile the blade is showing, so the same key adds to your deck with the Sideboard toggle off and to your sideboard with it on. You hear the new card count read back, the same as any other add, and "Cannot add another copy" in the cases where the game has that button switched off. On collection cards Ctrl+Enter keeps its existing meaning of fanning out the art styles.
...
- The Play button no longer goes missing from the Play group. Every time the home page finishes opening it runs a notification flow — the coroutine behind the season payouts, login grants, set announcements and the rest — and for as long as that flow is running the game deactivates the big Play button outright. If the mod scanned the screen in that window, and on a slow home page it usually did, the Play group came up one item short and stayed that way: the button reappearing is not a panel opening or closing, so nothing told the mod to look again. Backspacing out of the group and back in did not help either, because that is pure navigation and never rescans. The only thing that fixed it was some other navigator taking over and handing back — opening and closing the escape menu, say — which forced a fresh scan by accident. The mod now watches the button itself and rescans the moment it comes back.
...
- The mod now speaks even when no screen reader is running. Prism falls through to Narrator's voice or the Windows system voice, where Tolk simply went silent. That makes the mod usable while a screen reader is restarting, and gives a new user something to hear before they have one set up.
...
- Battlefield focus no longer slides onto a different card behind your back. Position in a row was a plain counter into a list that gets rebuilt and re-sorted left-to-right on every refresh — and pressing Enter forces exactly such a refresh. So acting on one of several identical permanents moved the ground under you: the stack split into two entries, or the game simply slid the permanents around as one tapped or stepped forward to attack, and "the third one" now meant a different creature. You could not work through five tokens by pressing Enter and arrowing along; every press meant working out where you had ended up. Focus is now pinned to the card itself rather than to its slot, and is re-found after each rebuild — including when the game merges your card back into a stack, where focus follows to the entry that now represents it. This is the same approach that has been keeping the Tab order stable. Position numbers can still shift as the board re-sorts; what changed is that the card under your focus stays the card you were on.

- Ctrl+Enter on a stacked entry now works everywhere, not just in combat. Only the declare-attackers and declare-blockers steps accept the game's stack-count badge — every other situation refuses it outright, so choosing four of your six Tentacles as targets, or tapping three of five identical creatures, meant one press per copy with a re-orientation in between. Where the badge is refused, the mod now clicks the stack's cards one after another instead: the same clicks a sighted player makes on a fanned-out stack, sent one at a time and each one accepted or refused by the game's own logic exactly as a mouse click would be. Each click waits for the game to be ready for it rather than going out on a timer — picking targets is a round trip to the server, and the game refuses every click made while one is still in flight, so clicks sent on a fixed rhythm were thrown away and only the first of them selected anything. Copies you already picked yourself are left alone instead of being un-picked. The run stops the moment the game moves on — once an effect has the number of targets it asked for, the remaining clicks are dropped rather than landing in whatever came next — and reports what actually happened ("3 of 5 clicked"). Any key press cancels a run in progress. Needs the "Battlefield stacking" setting; on a card that is not stacked, Ctrl+Enter is still just Enter.

- Stepping to the next copy after acting on one no longer hands you back a copy you already used. When you press Enter on one of several identical permanents, focus moves on to the next one — but it moved to the next card that merely shared the *name*, with no check on what state that card was in. Once a few copies were declared, that regularly meant landing back on a creature already set to attack, where the next Enter quietly took it out of combat again. Which copy is untouched is now decided by comparing states: a copy still sitting in the state the clicked card was in before you pressed Enter is one you have not used, and a copy already in the state it ended up in is one you have. When every copy is done, focus simply stays where it is instead of moving somewhere wrong.

- Selection state moved to the front of the battlefield announcement, straight after the card name. When you are picking several targets out of a wide row, whether a copy is already chosen is the one thing you are listening for, and it used to sit behind combat state, attachments and targeting — far enough back that checking it cost a full read of the line.

- Costs you pay while casting a spell — ward, and additional costs like "discard a card" — can now actually be paid. Targeting a creature with "Ward — discard a card" and answering Yes left you stuck: Tab reported nothing to choose from, Enter on a card in hand said it could not be played, and cancelling the spell was the only way out. The mod recognized "pick a card" states by the game's counted confirm button ("1 confirm"), but a cost paid mid-cast shows no such button — the game promotes Cancel to the primary button instead — and it puts no visible highlight on the cards either, so both of the ways the mod looks for a choice came up empty and Enter fell back to trying to cast the card. The mod now asks the game's own interaction logic which cards it would accept a click on, the same question a mouse click answers, so those cards are listed on Tab and a single Enter pays the cost. The instruction the game shows ("Discard a card") is read out when the payment starts, as it already was for discards that happen on resolution.

- Space during a Yes/No prompt no longer clicks a button belonging to something else. On prompts like the ward question, Space fell through to the duel's main prompt button, which at that moment was still the targeting confirm underneath the dialog — so the answer you had focused was never sent and the prompt just sat there. Space now activates the focused answer, the same as Enter, matching how it already worked for the other direct-choice prompts.

- "You may cast the exiled card" prompts can be turned down again. Chandra, Torch of Defiance's plus one exiles a card and offers it to you; if you could not pay for it, there was no way to say no. Space cast it, the spell then asked for a target you did not have, cancelling put the card back, and the offer came straight back — round and round. Two things were wrong. First, the confirm safety on these prompts was only pretending: it announced "press Space again to confirm", but the game's own keyboard handling had already taken that same press and cast the card, so the warning described a decision that was over. The mod now keeps the game out of Space for the whole visit to that prompt, so the first press really does only warn. Second, the escape hatch added in v1.2 for Tinybones-style "may cast" triggers only recognised the trap when a target had already been picked for you. On Chandra's version nothing is selected, so it did not match, and Backspace fell through to a cancel button that these prompts do not have — it did nothing at all. Backspace now covers both shapes: press it twice and the trigger is declined, which for Chandra means she deals her two damage instead. The prompt's hint (Ctrl+F1) mentions Backspace now, which it previously did not.

- The card a "you may cast" prompt is offering can now be heard. Tab in a card-selection browser walks only the cards the game will accept a click on, which is right in a library search where three cards out of eighty are legal picks — but when the exiled card turns out to be uncastable, nothing at all is legal, and the filter hid the browser's entire contents. Tab was a dead key: press after press produced silence, and the only thing that eventually happened was Tab leaving for another zone. So you were asked whether to cast a card you were never told the name of. When no card in the browser passes the filter, all of them are offered instead, and the card is read out on entry rather than waiting for a keypress. Where some cards are legal, Tab still skips the rest exactly as before.

- Targeting a player with V, Arrow Right, Enter no longer hits whatever was on the battlefield instead. Whether this worked came down to which zone you happened to be in before pressing V. The player info zone only moves focus onto a portrait; it does not take over the duel's zone ownership. So if you were last on the battlefield, the handler that runs first still believed the battlefield owned Enter, and during a targeting prompt it clicked the card you had been parked on — announcing that card as selected, which sounds exactly like a confirmation. A "choose any target" burn spell went into your own blocker; you found out when the damage resolved. Coming from your hand via Tab, that same handler stood aside and the press reached the player, which is why the shortcut looked reliable most of the time. The player info zone now holds on to Enter for as long as it has focus.

- Enter on a player who is not a legal target no longer toggles the opponent's mute. Once the press above had spent the targeting prompt, pressing V, Arrow Right, Enter again fell through to the mute switch, so a second attempt at targeting silently muted your opponent and a third unmuted them, with nothing said either way. While the game is asking for targets, Enter on a player either targets that player or says it cannot — it never reaches emotes or mute.

- Cards your opponent reveals are now spoken — "Opponent Revealed Llanowar Elves", with the rules text for up to three cards at a time. Explore, "reveal the top card of your library", and the hand peeks that some black spells cause all show the card face-up to both players, and all of them were silent: a sighted opponent-facing display appeared, sat there, and vanished again with nothing said, so a public piece of information was simply not available to you. The reveal announcement had in fact been written long ago, but it read a `CardName` field off the event that no reveal event has ever carried, so it could never produce a single word. Reveals are also not card movements — the card never leaves the library or hand, the game only shows a copy of it — so the zone-transfer machinery that announces draws, plays and deaths never saw them either. They are now read from the reveal event itself. Only the opponent's reveals are announced; your own already come through the browser you are looking at, and saying them twice would be worse than not at all. On a mass reveal, names only — a whole revealed hand with every ability text attached is not something anyone can listen through — and the "Brief opponent announcements" setting (F2) drops the rules text entirely, as it does elsewhere. Found and diagnosed by Michael Taboada (**@lilmike**, PR #113), who identified the event and the exact fields it hides the card behind; reimplemented here on the announcer's existing reflection helpers so that the second reveal event type the game sends cannot break it.
...
- The profile's collection line finally says what it is about. It used to read as a bare percentage with no set name — not sloppiness in the game, but a dead end in its code: the profile badge is built through `SetBadge.Init()`, the parameterless overload that only grabs the animator, so the tooltip the mod was reading the set name from is never filled in, and neither is the badge's own expansion code. The line is now read from the same `SetCollectionController` the badge is fed from, and says the set name and the actual card counts: "Collection: Tarkir Dragonstorm, 187 of 286 cards, 65 percent." It also stops being ambiguous about what the percentage measures — once you own one of every card in a set, the game silently reuses the same label for playset (4-of) completion, so a set at that stage now announces "complete, playsets 412 of 1144" instead of a percentage that quietly changed meaning.
