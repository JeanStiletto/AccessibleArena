...
- Ctrl+Enter on a stack works repeatedly within the same Declare Attackers / Blockers session. Previously the first press selected the whole stack but every subsequent press deselected only one card — once the action button showed a count ("3 Angreifer"), `HotHighlightNavigator` mistook the phase for selection mode and intercepted the Enter before `BattlefieldNavigator`'s stack handler could run. Ctrl+Enter is now reserved for the stack handler regardless of the button text, so toggling a 3-Goblin stack a second time deselects all three.
