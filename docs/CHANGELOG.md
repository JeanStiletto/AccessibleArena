...
- New Ctrl+Enter shortcut on a stacked battlefield card invokes the game's stack-count badge click — selecting/toggling the whole stack at once instead of one card at a time. Mirrors the small "xN" badge sighted players can click. Works in any workflow that accepts stack clicks (Declare Attackers, Declare Blockers, Select Targets, Group/Selection workflows, etc.) — the bridge asks the live workflow's `CanClickStack` and dispatches via `GameInteractionSystem.OnStackClicked`. Announces "Stack selection not available right now" when the current workflow rejects it; falls back to plain Enter for non-stacked cards.
- During combat, Ctrl+Enter on a stacked card assigns or removes the whole stack as attackers or blockers in a single press — e.g. send all 5 Tentakel to attack at once, or pull them all back with one more Ctrl+Enter.
...
- Faction-based event pages (e.g. Strixhaven Sealed) are now recognized as event screens — title is announced, and Left/Right info-block navigation works the same as Jump In and other events. Previously the V2 `FactionalizedEventTemplate` controller was unknown to the mod, so the screen showed up as "controller: none" with no event context extraction.
- Faction-event description is now navigable per faction. The dynamic description panel (`_currentFactionDescription`) shows a wall combining all faction summaries — the mod was reading it as one long info block. That panel's text is now suppressed and replaced by one generic intro block (pack counts, deckbuilding rules, prizes) plus one info block per faction, derived by resolving the shared `EventPageDescriptionLoc` once and splitting it on each faction's `FactionSelectShortNameLoc` (e.g. "Silverquill", "Silberkiel") which the wall uses as section headers. Resolution goes through `Languages.ActiveLocProvider` so the split is language-agnostic.
- Faction-tile buttons announce which faction is currently selected. The active tile's label gets a ", selected" suffix (read from `FactionEventContext.SelectedFaction`), and clicking a different tile triggers a rescan so the new selection is picked up on the next navigation pass.
- Event-page payment buttons now include the currency name. The game writes only the numeric quantity (e.g. "2000") to the button text and conveys the currency type via a sprite icon, which is invisible to screen readers; the mod now identifies which `MainButtonComponent` slot the button belongs to (`_payWithGemsButton` / `_payWithGoldButton`) and announces "2000 Edelsteine" / "2000 Gold". Event-token and Play/Start buttons keep their existing localized text.
- Mastery reward levels no longer announce the quantity twice (e.g. "Stufe 26: 600x 600 Edelsteine" → "Stufe 26: 600 Edelsteine"). The game's localized `RewardMainText` already embeds the count for currency and high-rarity card rewards, so the mod's own `{quantity}x` prefix was duplicating it; the prefix is now suppressed when the reward text already starts with the quantity.
- Opponent's exile zone (Shift+X) and command zone (Shift+W) now find their content. MTGA stores both as a shared wrapper with per-player sub-holders named "Exile PlayerId: #N" / "Command PlayerId: #N"; the old code looked for a non-existent "OpponentExile" GameObject and always announced an empty zone. Sub-holders are now discovered by their stable names and mapped to the correct side via the Hand/Library owner. Revealed cards in the opponent's hand (Duress/Inquisitor-style exposure) and cards the opponent may play from exile (Plot/Foretell, castable-from-exile effects) are now navigable under Shift+C alongside the hand count; Shift+C still announces count only when nothing is revealed, matching the library (Shift+D) pattern.
- I (extended card info) menu now shows the mana cost of castable linked faces — Adventure, Prepared Spell, Split halves, MDFC back, Room halves, Omen, Prototype. The cost was previously missing for every linked face because `CardPrintingData` exposes its cost as `CastingCost` (a structured ManaQuantity list) while the extractor only looked for `PrintedCastingCost` / a string. Transform-style backs (DFC back, meld, specialize child) remain cost-less since you never pay those.
...
- Hand size and index now count only real hand cards. Cards castable from other zones (flashback, foretell, commander) are visually placed in the hand holder but no longer inflate the announced hand size; navigating onto one suppresses the position since its origin-zone annotation already conveys where it lives. Applies to both own hand (C) and opponent hand (Shift+C).
