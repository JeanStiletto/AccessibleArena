...
- Opponent's hand (Shift+C) no longer lists cards that have left the hand. Cards the opponent has cast (their CDC lingers under the OpponentHand holder with `IsDisplayedFaceUp=true` even after moving to graveyard / exile / battlefield) used to stay in the revealed-cards list, producing a "Opponent's hand, 2 cards. … 1 of 3" mismatch where the displayed count and the navigable list disagreed and a card the opponent had already cast still appeared as if it were in their hand. The OpponentHand list now applies the same `IsRealHandCard` model-zone check the hand count already used, so list and count agree.
- Tab navigation no longer drops castable hand cards. MTGA's visual HotHighlight isn't reliably refreshed on hand cards in transitional states (notably immediately after a land play, where the battlefield creature's activated-ability highlight stays but the player's still-castable hand cards lose their visual cue), and the mod's scan only matched GameObjects whose names contained the literal substring `"HotHighlight"`, so Tab would cycle just the one item. For hand cards, the scan is now supplemented with a lookup into MTGA's own `ActionsAvailableWorkflow`: the workflow tracks a `GreInteraction` per available action with a pre-computed `CanAffordToCast` bool (set when the interaction is constructed from `Action.CanAffordToCast()`), and we include any hand card that has a playable interaction (any `Cast*` variant — including split halves, adventure, MDFC, prototype, Rooms, Omens — plus plain `Activate` for cycling/channel/foretell-the-card/plot-the-card/ninjutsu/etc., plus `Special`/`SpecialTurnFaceUp` for conspire/monarch/morph-flip) that the workflow itself considers affordable. Mana abilities (`ActivateMana`), land drops (`Play*`), and payment subactions are filtered out so Tab still only cycles meaningful actions. No mod-side affordability heuristics — the game answers the question.
