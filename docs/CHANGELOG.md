...
- Battlefield creature P/T now subtracts live damage so the announced toughness reflects how much damage the creature can still take. A 3/3 that took 2 damage in a fight announces as `3/1` instead of `3/3`. +1/+1 and -1/-1 counters were already folded into the toughness via the game's effect-overrides path; only damage was missing. Toughness is clamped at 0 (lethal-damaged but indestructible creature reads `3/0`); non-numeric toughness like `*` is left alone.
