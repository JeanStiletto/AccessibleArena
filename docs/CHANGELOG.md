...
- Faction-event description is now navigable per faction. The dynamic description panel (`_currentFactionDescription`) shows a wall combining all faction summaries — the mod was reading it as one long info block. That panel's text is now suppressed and replaced by one info block per faction, derived by resolving the shared `EventPageDescriptionLoc` once and splitting it on each faction's `FactionSelectShortNameLoc` (e.g. "Silverquill", "Silberkiel") which the wall uses as section headers. Resolution goes through `Languages.ActiveLocProvider` so the split is language-agnostic.
