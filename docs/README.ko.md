...
4. `prism.dll`이 MTGA 루트 폴더에 있는지 확인
5. MTG Arena 실행

<h2>제거</h2>

설치 프로그램을 다시 실행하세요. 모드가 이미 설치되어 있다면 제거 옵션이 제공됩니다. 선택적으로 MelonLoader도 제거할 수 있습니다. 수동으로 제거하려면 `Mods\` 폴더에서 `AccessibleArena.dll`을 삭제하고 MTGA 루트 폴더에서 `prism.dll`을 제거하세요.
...
- MTG Arena를 실행하기 전에 스크린 리더가 실행 중인지 확인하세요 (없으면 모드가 Windows 시스템 음성으로 대체합니다)
- `prism.dll`이 MTGA 루트 폴더에 있는지 확인하세요 (설치 프로그램이 자동으로 배치함)
- MelonLoader 로그에 선택된 출력이 기록됩니다. `[Speech] ready` 줄을 찾아보세요
...
음성은 Prism 라이브러리를 통해 나갑니다. Prism은 NVDA, JAWS, 내레이터, ZDSR, PC-Talker, BoyPC Reader, Sense Reader, ZoomText, Windows 시스템 음성을 하나의 인터페이스로 연결하므로, 스크린 리더가 실행되지 않아도 모드가 말합니다. 출력은 모드 설정(F2)의 '음성 출력'에서 직접 고를 수 있습니다. 여기서는 Windows에서 NVDA와 JAWS로 테스트합니다. 나머지 출력은 Prism에서 오며 제가 테스트하지 못했으니 문제가 있으면 알려주세요. Mac과 Linux는 여전히 손이 닿지 않지만, 이제 걸림돌은 음성 라이브러리가 아니라 모드 로더와 게임 클라이언트입니다. 다른 플랫폼이나 제가 없는 스크린 리더로 테스트를 도와주실 수 있다면 주저 없이 연락해 주세요.
