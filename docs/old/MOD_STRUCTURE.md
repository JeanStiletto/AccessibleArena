...
- **Spell cast announcements** (via Harmony) - "Cast [card name]" when any spell goes on stack
- **Spell resolution** (via Harmony) - Announces when spells resolve from stack
- **Phase announcements** (via Harmony) - Main phases, combat steps (attackers, blockers, damage)
- **Combat announcements** (via Harmony) - Combat begins, attacker declared/removed
- **Discard selection** - Enter toggles, Space submits, announces required/selected count
- **Combat phase support (partial)** - Space to attack all, blocker state detection working, attacker detection WIP
...
- `NavContentController` - FinishOpen/FinishClose (after animations), BeginOpen/BeginClose (at start)
- `NavContentController.IsOpen` setter - Backup detection
- `SettingsMenu.Open()` / `Close()` - Works correctly (Open has 7 bool parameters)
- `SettingsMenu.IsOpen` setter - Backup detection
- `DeckSelectBlade.Show(EventContext, DeckFormat, Action)` / `Hide()` - Works correctly
- `DeckSelectBlade.IsShowing` setter - Backup detection

**Harmony Flag Approach (Key Innovation):**
The critical insight: Harmony events are 100% reliable but reflection-based panel detection
during rescan was failing. Solution: Set overlay flags immediately when Harmony fires.

```csharp
// In OnPanelStateChangedExternal():
if (panelTypeName.Contains("SettingsMenu") && isOpen)
    _settingsOverlayActive = true;

// In IsInForegroundPanel():
bool overlayActive = _foregroundPanel != null || _settingsOverlayActive || _deckSelectOverlayActive;
```

This ensures background elements (NavBar, HomePage) are filtered from the first rescan,
without waiting for panel detection via reflection.

**Type Discovery:**
`PanelStatePatch.DiscoverPanelTypes()` logs all types with IsOpen/Show/Hide at startup.
Useful for finding new panel controllers to patch.

**Integration:**
- `NavigatorManager` subscribes to `PanelStatePatch.OnPanelStateChanged` event
- Routes panel changes to active navigator's `OnPanelStateChangedExternal()` method
- GeneralMenuNavigator sets overlay flags and triggers rescan when panel changes detected
