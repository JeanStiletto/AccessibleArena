...
**Lands**
- Play correctly, skip targeting mode (detected via card type)

**Creatures**
- Play correctly, skip targeting mode (detected via stack increase event)

**Targeted spells**
- Tab targeting mode activates, cycles through valid targets only
...
**TargetNavigator.cs**
- Path: `src/Core/Services/TargetNavigator.cs`
- Purpose: Target cycling service (~340 lines)

**UIActivator.cs**
- Path: `src/Core/Services/UIActivator.cs`
- Purpose: `IsTargetingModeActive()` detection, `PlayCardViaTwoClick()` integration

**TargetInfo.cs**
- Path: `src/Core/Models/TargetInfo.cs`
- Purpose: Target data model and CardTargetType enum
